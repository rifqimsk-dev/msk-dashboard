...
func TotalAmountHandler(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var currentMonth, previousMonth sql.NullFloat64
		...
		resultCurrent := 0.0
		if currentMonth.Valid {
			resultCurrent = currentMonth.Float64
		}

		resultPrevious := 0.0
		if previousMonth.Valid {
			resultPrevious = previousMonth.Float64
		}

		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(map[string]float64{
			"total_amount":             resultCurrent,
			"total_amount_sebelumnya":  resultPrevious,
		})
	}
}
